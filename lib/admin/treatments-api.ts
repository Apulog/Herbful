...
import { updateSymptomsForTreatment, removeTreatmentFromSymptoms } from "./symptoms-api"
...
async function updateTreatmentRating(treatmentId: string): Promise<void> {
  try {
    ...
    const totalRating = treatmentReviews.reduce((sum, review) => sum + (review as any).rating, 0)
...
export async function getTreatments(params: {
  ...
  limit: number
  search?: string
}): Promise<{ treatments: Treatment[]; totalPages: number; totalCount: number }> {
  try {
    ...
    if (params.search) {
      ...
      treatments = treatments.filter(
        (treatment) =>
          ...
          (treatment.benefits && treatment.benefits.some((benefit) => benefit.toLowerCase().includes(searchLower))) ||
          (treatment.symptoms && treatment.symptoms.some((symptom) => symptom.toLowerCase().includes(searchLower)))
      )
    }
...
export async function getTreatment(id: string): Promise<Treatment> {
  try {
    ...
    if (!snapshot.exists()) {
      console.error(`Treatment with ID "${id}" not found in Firebase at path: ${TREATMENTS_PATH}/${id}`)
...
export async function createTreatment(
  data: Omit<Treatment, "id" | "averageRating" | "totalReviews" | "createdAt" | "updatedAt">
): Promise<Treatment> {
  try {
    const id = generateId()
